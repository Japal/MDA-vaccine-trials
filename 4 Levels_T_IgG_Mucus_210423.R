# Levels of total IgG in mucus data analysis

dat <- read.csv("Levels_T_IgG_Mucus.csv", as.is = FALSE)
...
summary(avi.red.lm) # R2 = 0.03285, 3.28% variability in cFEC.red explained by PC1 and PC2
avi.red.lm <- lm(dat.supp[,10] ~ log.avi.pca$x[,1:2])
summary(avi.red.lm) # R2 = 0.03269, 3.27% variability in WB.red explained by PC1 and PC2

cor(dat.supp) # Correlations between any reduction variable and indexes are < 0.19
