# Levels of total IgG in serum data analysis 

dat <- read.csv("Levels_T_IgG_Serum.csv", as.is = FALSE)
...
# There are some negative response values, mostly in Trial 3 and affecting MIF, however removing trial 3 the biplots looks very similar
# In order to be able to take logs we find the lowest value and add its absolute value + 10^3 (original data have 3 decimal digits)
# Still MIF deviating, so the "imputation" is having negligible effect here

mins <- apply(lev,2,min)
tmp <- which(mins <= 0)
tmp.lev <- lev[,tmp]
mins <- apply(tmp.lev,2,min)
lev2 <- lev
lev2[,tmp] <- t(apply(tmp.lev,1,function(x) x + abs(mins) + 10^(-3)))
log.lev <- log(lev2) # Logged data
...
summary(avi.red.lm) # R2 = 0.0163, 1.63% variability in cFEC.red explained by PC1 and PC2
avi.red.lm <- lm(dat.supp[,10] ~ log.avi.pca$x[,1:2])
summary(avi.red.lm) # R2 = 0.03966, 3.97% variability in WB.red explained by PC1 and PC2

cor(dat.supp) # Correlations between any reduction variable and indexes are < 0.24
