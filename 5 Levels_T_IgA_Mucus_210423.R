# Levels of total IgA in mucus data analysis 

dat <- read.csv("Levels_T_IgA_Mucus.csv", as.is = FALSE)
...
# Flip vertical axis to agree with arrangement in previous biplots
res.pca$var$coord[,2] <- res.pca$var$coord[,2]*(-1)
res.pca$ind$coord[,2] <- res.pca$ind$coord[,2]*(-1)
...
summary(avi.red.lm) # R2 = 0.01159, 1.16% variability in cFEC.red explained by PC1 and PC2
avi.red.lm <- lm(dat.supp[,10] ~ log.avi.pca$x[,1:2])
summary(avi.red.lm) # R2 = 0.03938, 3.94% variability in WB.red explained by PC1 and PC2

cor(dat.supp) # Correlations between any reduction variable and indexes are < 0.17
